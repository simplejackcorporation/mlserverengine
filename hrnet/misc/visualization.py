...
def showAnns(image, points_array):
    """
    ...
    for points in points_array:
        for i, point in enumerate(points):
            ...
            y, x = point[1], point[0]
            image = cv2.circle(image, (int(x), int(y)), circle_size, tuple(colors[i % len(colors)]), -1)

        yx_formated_points = [(item[1], item[0]) for item in points]
        image = draw_skeleton(image, yx_formated_points, joints_dict()["coco"]['skeleton'], is_use_confidence=False)
...
def draw_skeleton(image, points, skeleton, color_palette='Set2', palette_samples=8, person_index=0,
                  confidence_threshold=0.5, is_use_confidence=True):
    """
...
def check_video_rotation(filename):
    ...
    print("filename before probe", filename)
